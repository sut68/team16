...
const openEditModal = (user: any) => {
  ...
  if (user.student_profile?.[0]) {
    ...
    form.value.admin_first_name = a.admin_first_name; 
    form.value.admin_last_name = a.admin_last_name;
          ...
          <tbody>
            <tr v-for="u in users" :key="u.ID" class="hover:bg-gray-50 border-b">
              ...
              <td class="py-3 px-6">
                ...
                <span v-else-if="u.admin_profile?.length">{{ u.admin_profile[0].admin_first_name }} {{ u.admin_profile[0].admin_last_name }}</span>
              </td>
      ...
      <div class="bg-white rounded-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto flex flex-col">
        ...
        <div class="p-6 space-y-4">
          ...
          <div v-else class="grid grid-cols-2 gap-4 animate-fade-in">
            <div class="form-control"><label class="label">ชื่อจริง</label><input v-model="form.admin_first_name" class="input input-bordered"/></div>
            <div class="form-control"><label class="label">นามสกุล</label><input v-model="form.admin_last_name" class="input input-bordered"/></div>
