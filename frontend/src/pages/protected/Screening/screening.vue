...
// Interface ปรับปรุงให้รองรับการแสดงผล
interface DocumentItem {
  id: number;
  title: string;
  applicant: string;
  submission_date: string; // เปลี่ยนชื่อให้สื่อความหมาย
  round_label: string; // "1/2567" สำหรับโชว์
  status: 'pending' | 'approved' | 'rejected' | 'request-change';
  rejection_reason?: string;
  raw_data?: ScreeningResponse;
  created_at_date?: Date;
  semaster: Partial<SemasterResponse>; // รองรับข้อมูลไม่ครบ
}
...
const filterRound = ref('all'); // เก็บค่า 'round' เฉยๆ (เช่น '1', '2')

// UI States
const isModalOpen = ref(false);
const isFilterOpen = ref(false); // ควบคุมการเปิดปิดเมนู Filter
const selectedDocument = ref<any | null>(null);

// Options สำหรับ Dropdown (คำนวณอัตโนมัติ)
...
const fetchData = async () => {
  ...
  try {
    ...
    allItems.value = response.map((item: any) => {
      ...
      // จัดการข้อมูลเทอม/ปี ให้ปลอดภัย
      const termVal = sem.term || sem.Term || '';
      const yearVal = sem.academic_year || sem.AcademicYear || '';
      const roundVal = sem.round || '1'; // สมมติถ้าไม่มีให้เป็น 1
      ...
      return {
        ...
        round_label: (termVal && yearVal) ? `${termVal}/${yearVal}` : 'ไม่ระบุ',
        semaster: semData,
    ...
    // Extract Unique Years เพื่อใส่ใน Filter แรก
...
// --- Cascading Filter Logic (Watchers) ---
...
watch(filterTerm, (newTerm) => {
  ...
  if (filterYear.value !== 'all' && newTerm !== 'all') {
    // ใน case นี้เราใช้ round จากข้อมูล (หรือถ้าไม่มีให้ใช้เทอมแทนตาม Logic เดิมของคุณ)
    // ตรงนี้ผม map ให้ดูว่าเราสามารถกรองลึกไปถึง "รอบที่เปิดรับ" ได้ถ้า API ส่งมา
    // แต่ถ้า API ส่งมาแค่เทอม ให้ข้ามส่วนนี้ไป หรือใช้ Logic เดิม
    const rounds = [...new Set(allItems.value
      .filter(i => i.semaster?.academic_year === filterYear.value && i.semaster?.term === newTerm)
      .map(i => i.semaster?.round || '1') // สมมติว่ามี field round
    )];
...
watch(activeTab, () => {
  ...
  // ปิด Filter popup เมื่อเปลี่ยน Tab
...
const filteredItems = computed(() => {
  ...
  // 3. Filter Round (ถ้ามี)
  if (filterRound.value !== 'all') {
     // ต้องมั่นใจว่าใน DocumentItem มี field round หรือเทียบกับ logic ที่ถูกต้อง
     // ตัวอย่าง: result = result.filter(item => item.semaster?.round == filterRound.value);
  }

  // 4. Status (เฉพาะ History)
          ...
          <div class="card-body p-4 md:p-5 flex flex-row items-center justify-between min-h-[6rem]">
            ...
            <div class="flex flex-col overflow-hidden pr-2">
              <div class="flex flex-wrap items-center gap-2 mb-1">
                ...
                <span v-if="item.status === 'request-change'"
