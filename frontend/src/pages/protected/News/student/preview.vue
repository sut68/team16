...
const getScholarshipStatus = (openDateStr: string | undefined, closeDateStr: string | undefined) => {
    // Default
    if (!openDateStr || !closeDateStr) return { label: 'ไม่ระบุ', color: 'gray', icon: AlertCircle, canApply: false };
    ...
    if (now < openDate) return { label: 'เร็วๆ นี้', color: 'amber', icon: Clock, canApply: false }; // ยังไม่เปิด
    else if (now > closeDate) return { label: 'ปิดรับสมัคร', color: 'red', icon: CheckCircle2, canApply: false }; // ปิดแล้ว
    else return { label: 'กำลังเปิดรับ', color: 'green', icon: CheckCircle2, canApply: true }; // เปิดอยู่
};

// คำนวณ % หลอด (Logic เดิมของพี่)
const getProgressPercent = (startStr: string, endStr: string) => {
    ...
    // หมายเหตุ: Logic เดิมพี่คูณ 250 ผมคงไว้ตามเดิมนะครับ
    const percent = (elapsed / totalDuration) * 250;
    return Math.min(100, Math.max(0, percent));
};

// คำนวณวันคงเหลือ (Logic เดิมของพี่)
...
const fetchDetail = async () => {
  if (!props.id) { error.value = "Invalid News ID"; loading.value = false; return; }
  loading.value = true;
  try {
  ...
  } catch (err) { console.error(err); error.value = "เกิดข้อผิดพลาดในการโหลด"; } 
...
<template>
  <div class="fixed inset-0 z-[1000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in font-prompt" @click.self="goBack">
      ...
      <div class="bg-[#F8FAFC] w-full max-w-5xl h-[90vh] rounded-[32px] shadow-2xl overflow-hidden flex flex-col relative animate-scale-in">
          ...
          <div class="flex-1 overflow-y-auto custom-scrollbar bg-[#F8FAFC]">
            ...
            <div v-else-if="error" class="h-full flex flex-col items-center justify-center text-center text-slate-500">
                ...
                <p>{{ error }}</p>
                <button @click="goBack" class="mt-4 text-[#1e3a8a] underline">ปิดหน้าต่าง</button>
            </div>
          ...
          <div v-if="news && news.scholarship" class="p-4 bg-white border-t border-slate-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20 flex justify-end gap-3 items-center">
              
              <div class="hidden md:flex flex-col items-end mr-2 text-right">
                  <span class="text-xs text-slate-400">ปิดรับสมัคร: {{ formatDate(news.scholarship.close_date) }}</span>
              </div>

              <button 
                  @click="handleApply"
                  :disabled="!getScholarshipStatus(news.scholarship.open_date, news.scholarship.close_date).canApply"
                  class="flex-1 md:flex-none btn bg-[#1e3a8a] hover:bg-[#172554] text-white border-none rounded-xl px-8 shadow-lg shadow-blue-200 flex items-center gap-2 transition-all active:scale-95 disabled:bg-slate-300 disabled:text-slate-500 disabled:shadow-none"
              >
                  <Send :size="18" />
                  <span>
                      {{ getScholarshipStatus(news.scholarship.open_date, news.scholarship.close_date).canApply ? 'สมัครทุนที่นี่' : 'ไม่อยู่ในช่วงรับสมัคร' }}
                  </span>
              </button>
          </div>
